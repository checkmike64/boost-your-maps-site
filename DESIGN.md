...
- Homepage CTAs jump to the Visibility Report section. That panel contains the full explanation, the action button, and a framed sample report.
...
Left spine for story sections (why, how, what-we-handle); centered only for symmetric grids (proof, pricing, FAQ, final). Between story sections, a centered 46px dashed connector ending in an 8px red dot hands one section into the next — same visual language as the dashed route line behind the three steps. Section rhythm: 96px standard, 64px around the flattened straight-talk passage and the report panel. Only the three steps are numbered (red numerals in outlined circles); the what-we-handle cards use small red pin-arrow glyphs instead. The main mascot belongs in the hero. The Visibility Report panel uses the real report sample as its visual proof, and the final CTA stays text-only.
...
## 7. Do's and Don'ts
DO: one red per viewport; one clear Visibility Report path; three text sizes; route connectors between sections; the four playful moments; the interactive 3-Pack map (best element on the page — never remove).
...
## 8. Responsive
Breakpoint 880px: grids stack, route connectors hide, plain nav links hide, the report sample moves below its copy, and sections drop to 72/52px. At 420px, sections tighten, primary CTAs span the content width, and the report thumbnail stays compact. At 380px, the sticky header keeps only the mascot mark beside its CTA. Touch targets ≥44px.

## 9. Agent Prompt Guide
"Build/restyle per Boost Your Maps DESIGN.md: white canvas, Gabarito 800 + Hanken Grotesk (three body sizes only), single deep red #C5221F accent on actions/numerals/mascot with green checks and one yellow h1 underline, 1px hairlines inside components, dashed route connectors with red dots between sections, flat simple pin mascot (dot eyes + smile, never detailed), homepage CTAs jump to the red-bordered gray Visibility Report panel, and that panel pairs its copy with a compact real report sample. Secondaries are text links. No colored bands, no offset shadows, no pills, no icon-box grids, no extra grays. Motion: 250ms cubic-bezier(.32,.72,0,1), with only a brief one-time mascot bob."
